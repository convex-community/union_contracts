...
contract stkCvxPrismaStrategyBase {
    ...
    ICurveV2Pool cvxPrismaPrismaSwap = ICurveV2Pool(CURVE_CVXPRISMA_PRISMA_POOL);
    ...
    function _swapEthCvx(uint256 amount, uint256 minAmountOut, bool ethToCvx)
        internal
        returns (uint256)
    {
        return
    ...
    function _swapEthPrisma(uint256 amount, uint256 minAmountOut, bool ethToPrisma)
    internal
    returns (uint256)
    {
        return
    ...
    /// @return amount of token obtained after the swap
    function _swapMkUsdPrisma(uint256 amount, uint256 minAmountOut, bool mkUsdToPrisma)
    internal
    returns (uint256)
    {
        return
            mkUsdPrismaSwap.exchange_underlying(
            mkUsdToPrisma ? PRISMAMKUSD_MKUSD_INDEX : PRISMAMKUSD_PRISMA_INDEX,
            mkUsdToPrisma ? PRISMAMKUSD_PRISMA_INDEX : PRISMAMKUSD_MKUSD_INDEX,
            amount,
            minAmountOut
        );
    }
    ...
    /// @return amount of token obtained after the swap
    function _swapMkUsdPrisma(uint256 amount, uint256 minAmountOut, bool cvxPrismaToPrisma)
    internal
    returns (uint256)
    {
        return
            cvxPrismaPrismaSwap.exchange(
            cvxPrismaToPrisma ? PRISMACVXPRISMA_CVXPRISMA_INDEX : PRISMACVXPRISMA_PRISMA_INDEX,
            cvxPrismaToPrisma ? PRISMACVXPRISMA_PRISMA_INDEX : PRISMACVXPRISMA_CVXPRISMA_INDEX,
            amount,
            minAmountOut
        );
    }
