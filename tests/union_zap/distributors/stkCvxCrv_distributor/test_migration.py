...
from tests.utils.constants import (
    UNION_CRV_V2, AIRFORCE_SAFE, UNION_CRV_DISTRIBUTOR_V2,
)
...
def test_migration(
    ...
    old_distributor_cvxcrv_balance = interface.IGenericVault(UNION_CRV_V2).balanceOfUnderlying(old_distributor)
    old_distributor_ucrv_balance = interface.IERC20(UNION_CRV_V2).balanceOf(old_distributor)
    original_distributor_cvxcrv_balance = vault.balanceOfUnderlying(distributor)
    interface.IGenericVault(UNION_CRV_V2).setWithdrawalPenalty(0, {"from": AIRFORCE_SAFE})
    ...
    tx = migration.migrate(proofs["claim"]["index"],
                      migration,
    ...
    assert approx(vault.balanceOfUnderlying(distributor) - original_distributor_cvxcrv_balance, old_distributor_cvxcrv_balance, 1e-12)
