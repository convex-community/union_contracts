...
def test_claim_as_underlying(
    ...
    fxs_zaps.setSwapOption(3, {"from": owner})
    ...
    fxs_amount = cvxfxs_to_fxs(CLAIM_AMOUNT * (1 - withdrawal_penalty))
    cvx_fxs_amount = CLAIM_AMOUNT * (1 - withdrawal_penalty)
    ...
    tx = distributor_zaps.claimFromDistributorAsFxs(
        proofs["claim"]["index"],
        alice.address,
        CLAIM_AMOUNT,
        proofs["proofs"],
...
def test_claim_as_eth(
    ...
    fxs_amount = cvxfxs_to_fxs(CLAIM_AMOUNT * (1 - withdrawal_penalty))
    eth_amount = fxs_to_eth(fxs_amount, 3)
