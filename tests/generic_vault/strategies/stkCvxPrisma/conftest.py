...
from ....utils.constants import (
    ...
    PRISMA_LOCKER,
    PRISMA, STABILITY_POOL, MKUSD_TOKEN, CONVEX_LOCKER, CVX,
)
...
def distribute_rewards(staking, alice, zaps):
    ...
    staking.approveRewardDistributor(PRISMA, PRISMA_LOCKER, True, {'from': staking_owner})
    staking.approveRewardDistributor(CVX, CONVEX_LOCKER, True, {'from': staking_owner})
    staking.approveRewardDistributor(MKUSD_TOKEN, STABILITY_POOL, True, {'from': staking_owner})
    ...
    interface.IERC20(MKUSD_TOKEN).approve(staking, 2 ** 256 - 1, {"from": STABILITY_POOL})
