...
def test_strategy_set_approvals(fn_isolation, alice, owner, vault, strategy, staking):
    interface.IERC20(CVXPRISMA).approve(staking, 0, {"from": strategy})
    strategy.setApprovals({"from": owner})
    assert interface.IERC20(CVXPRISMA).allowance(strategy, staking) == 2**256 - 1
...
def test_strategy_rescue_token(fn_isolation, alice, owner, vault, strategy):
    original_owner_balance = interface.IERC20(PRISMA).balanceOf(owner)
    interface.IERC20(PRISMA).transfer(strategy, 1e24, {"from": PRISMA_LOCKER})
    strategy.rescueToken(PRISMA, owner, 1e24, {"from": owner})
    assert interface.IERC20(PRISMA).balanceOf(owner) > original_owner_balance
