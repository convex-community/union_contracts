from brownie import accounts, interface, FXSMerkleDistributor, FXSSwapper, MerkleDistributor, UnionZap, DistributorZaps
...
def main():
    ...
    swaps = FXSSwapper.at("0x2dce36d00bb398cb01390a0769260b84edded929")
    swaps.updateDepositor(zap, {"from": deployer})
    swaps.transferOwnership(AIRFORCE_SAFE, {"from": deployer})

    fxs_distributor = FXSMerkleDistributor.at("0x11fe17c5ab68cc4cc6d3c281feddfff80bc1d4c7")
    fxs_distributor.updateDepositor(zap, {"from": deployer})
    fxs_distributor.updateAdmin(AIRFORCE_SAFE, {"from": deployer})
    ...
    distributor_zaps = DistributorZaps.deploy(FXS_ZAPS, fxs_distributor, FXS_VAULT, {"from": deployer}, publish_source=True)
