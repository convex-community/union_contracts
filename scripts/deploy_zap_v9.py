...
from tests.utils import CRV
from tests.utils.constants import CVX, FXS, CURVE_CVX_ETH_POOL, CURVE_FXS_ETH_POOL, CURVE_TRICRV_POOL
...
UCVX_DISTRIBUTOR = "0x6788234F40931Ca615b0D221c1afbf0Ec07AFCc5"
...
def main():
    ...
    zap = UnionZap.deploy({"from": deployer}, publish_source=publish)
    zap.setApprovals({"from": deployer})
    assert interface.IERC20(CRV).allowance(zap, CVXCRV_DEPOSIT) == 2**256 - 1

    zap.updateOutputToken(
        CRV, [CURVE_TRICRV_POOL, ADDRESS_ZERO, UCRV_DISTRIBUTOR], {"from": deployer}
    )
    zap.updateOutputToken(
